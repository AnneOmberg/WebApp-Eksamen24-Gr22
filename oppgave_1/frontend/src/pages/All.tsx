...
// const getLesson = async (courseSlug: string, lessonSlug: string): Promise<LessonType | undefined> => {
...
//   useEffect(() => {
//     const getContent = async () => {
//         const lessonDate = await getLesson(courseSlug, lessonSlug); // TypeScript knows this is a LessonType | undefined
//         const courseData = await getCourse(courseSlug); // Assuming getCourse is already typed correctly
//         const commentsData = await getComments(lessonSlug); // TypeScript knows this is a CommentType[]
...
//   const formIsValid = Object.values(fields).filter((val: any) => val?.length === 0);
...
//   const handleLessonFieldChange = (event: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>, index: number) => {
//     const { name, value } = event.target;
//     let text: any;
...
//   const changeCurrentLesson = (index: any) => {
...
//                     onChange={(event) => handleLessonFieldChange(event, currentLesson)}
//                   />
//                 </label>
//                 <label className="mb-4 flex flex-col" htmlFor="slug">
//                   <span className="mb-1 font-semibold">Slug*</span>
