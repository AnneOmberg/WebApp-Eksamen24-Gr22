...
export default function Info() {
  ...
  return (
    <div>
      <section>
        ...
        <section className="flex justify-between">
          ...
          <section>
            <p>
              <strong>Dato: </strong>
              {new Date(content.date).toLocaleDateString("no-NO", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
            ...
            <p>
              <strong>Lokasjon:</strong> {content.location}
            </p>
          </section>
        </section>
        <p className="mt-4">{content.description}</p>
        <p>
          <strong>Pris:</strong> {content.price},-
        </p>
        {content.status === true ? (
          <p>
          ...
          <p>
            <strong>Ledige plasser:</strong> {content.seats}
          </p>
        )}
        {!isAdmin && (
      ...
      <section>
        <h3>Liste over deltakere</h3>
        {content?.participants?.map(
          (person: { id: string; name: string; email: string }) => (
            <section
              className="w-1/12 bg-slate-300 mb-3 p-2 rounded"
              key={person.id}
            >
              <p>{person.name}</p>
            </section>
