...
export default function HappeningCard({ deleteHappening, happening}: HappeningCardProps) {
        ...
        >
          ...
          <section className="p-4">
            <ul className="mb-4">
              ...
              <li className="text-gray-700 mb-1"><strong>Lokasjon:</strong> {happening.location}</li>
              <li className="text-gray-700 mb-1"><strong>Pris:</strong> {happening.price},-</li>

              {happening.status === true ?
                <li className="text-gray-700 mb-1"><strong>Fullboket</strong></li>
                :
                <li className="text-gray-700 mb-1"><strong>Antall ledige plasser:</strong> {happening.seats}</li>
              }
              ...
              {!isAdmin && (
                <button className="border-2 bg-blue-500 px-4 py-2 rounded text-white hover:bg-blue-600">
                  {happening.status === false ?
                    <Link className="font-semibold underline" href={`/happenings/${happening.slug}/order`}>Kjøp biletter</Link> : <Link className="font-semibold underline" href={`/happenings/${happening.slug}/order`}>Sett deg opp på ventelise</Link>}
                </button>
              )}
              {isAdmin && (
                <button
